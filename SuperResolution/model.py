...
class ConvolutionalBlock(nn.Cell):
    """
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, batch_norm=False, activation=None):
        ...
        self.conv_block = nn.SequentialCell(*layers)
    ...
    def construct(self, input_img):
        ...
        """
        output = self.conv_block(input_img)  # (N, out_channels, w, h)

        return output


class SubPixelConvolutionalBlock(nn.Cell):
    """
    ...
    def construct(self, input_img):
        ...
        """
        output = self.conv(input_img)  # (N, n_channels * scaling factor^2, w, h)
...
class ResidualBlock(nn.Cell):
    ...
    def construct(self, input_img):
        ...
        """
        residual = input_img  # (N, n_channels, w, h)
        output = self.conv_block1(input_img)  # (N, n_channels, w, h)
...
class SRResNet(nn.Cell):
    ...
    def __init__(self, large_kernel_size=9, small_kernel_size=3, n_channels=64, n_blocks=16, scaling_factor=4):
        ...
        self.subpixel_convolutional_blocks = nn.SequentialCell(
...
class Generator(nn.Cell):
    ...
    def initialize_with_srresnet(self, srresnet_checkpoint):
        ...
        load_param_into_net(self.net, srresnet_params_dict)
...
class Discriminator(nn.Cell):
    ...
    def __init__(self, kernel_size=3, n_channels=64, n_blocks=8, fc_size=1024):
        ...
        self.conv_blocks = nn.SequentialCell(*conv_blocks)

        # An adaptive pool layer that resizes it to a standard size
        # For the default input size of 96 and 8 convolutional blocks, this will have no effect
        self.adaptive_pool = nn.AdaptiveAvgPool2d((6, 6))

        self.fc1 = nn.Dense(out_channels * 6 * 6, fc_size)

        self.leaky_relu = nn.LeakyReLU(0.2)

        self.fc2 = nn.Dense(1024, 1)
    ...
    def construct(self, imgs):
        ...
        :param imgs: high-resolution or super-resolution images which must be classified as such, 
...
class TruncatedVGG19(nn.Cell):
    ...
    def __init__(self, i, j):
        ...
        vgg19 = mindcv.models.vgg19(pretrained=True)

        maxpool_counter = 0
        conv_counter = 0
        truncate_at = 0
        # Iterate through the convolutional section ("features") of the VGG19
        for layer in vgg19.features.cells():
            truncate_at += 1
        ...
        self.truncated_vgg19 = nn.SequentialCell(*list(vgg19.features.cells())[:truncate_at + 1])
    ...
    def construct(self, input_img):
        ...
        """
        output = self.truncated_vgg19(input_img)  # (N, feature_map_channels, feature_map_w, feature_map_h)
