...
class SequenceDataset:
    ...
    def __next__(self):
        ...
        :returns: the next batch, containing:
            ...
            true target language lengths, typically the same as decoder_sequence_pad_length
        """
        ...
        source_data = pad_sequence(sequences=[mindspore.Tensor(s, mindspore.int64) for s in source_data],
                                   padding_value=self.bpe_model.subword_to_id('<PAD>'))
        target_data = pad_sequence(sequences=[mindspore.Tensor(t, mindspore.int64) for t in target_data],
                                   padding_value=self.bpe_model.subword_to_id('<PAD>'))

        # Convert lengths to tensors
        source_lengths = mindspore.Tensor(source_lengths, mindspore.int64)
        target_lengths = mindspore.Tensor(target_lengths, mindspore.int64)
