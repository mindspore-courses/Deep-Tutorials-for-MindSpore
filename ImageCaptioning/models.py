...
class Attention(nn.Cell):
    ...
    def __init__(self, encoder_dim, decoder_dim, attention_dim):
        ...
        """
        super().__init__()
        self.encoder_att = nn.Dense(encoder_dim, attention_dim)
        self.decoder_att = nn.Dense(decoder_dim, attention_dim)
        self.full_att = nn.Dense(attention_dim, 1)
        self.relu = nn.ReLU()
...
class DecoderWithAttention(nn.Cell):
    ...
    def init_weights(self):
        """
        initialize embedding layer and fc layer
        """
        self.embedding.embedding_table.set_data(initializer(Uniform(0.1),
                                                            self.embedding.embedding_table.shape,
                                                            self.embedding.embedding_table.dtype))
        self.fc.bias.set_data(initializer('zero', self.fc.bias.shape, self.fc.bias.dtype))
        self.fc.weight.set_data(initializer(Uniform(0.1),
                                            self.fc.weight.shape,
                                            self.fc.weight.dtype))
    ...
    def load_pretrained_embeddings(self, embeddings):
        ...
        """
        self.embedding.embedding_table = Parameter(embeddings)
    ...
    def fine_tune_embeddings(self, fine_tune=True):
        ...
        :param fine_tune: Allow?
        """
        for p in self.embedding.get_parameters():
            p.requires_grad = fine_tune
    ...
    def construct(self, encoder_out, encoded_captions, caption_lengths):
        ...
        predictions = ops.zeros((batch_size, max(decode_lengths), vocab_size))
        alphas = ops.zeros((batch_size, max(decode_lengths), num_pixels))

        # At each time-step, decode by
        # attention-weighing the encoder's output based on the decoder's previous hidden state output
        # then generate a new word in the decoder with the previous word and the attention weighted encoding
        for t in range(max(decode_lengths)):
            batch_size_t = sum(l for l in decode_lengths if l > t)
            attention_weighted_encoding, alpha = self.attention(encoder_out[:batch_size_t],
                                                                h[:batch_size_t])
            gate = self.sigmoid(self.f_beta(h[:batch_size_t]))  # gating scalar, (batch_size_t, encoder_dim)
            attention_weighted_encoding = gate * attention_weighted_encoding
            h, c = self.decode_step(
                ops.cat([embeddings[:batch_size_t, t, :], attention_weighted_encoding], axis=1),
