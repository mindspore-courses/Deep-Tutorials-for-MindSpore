...
rgb_weights = mindspore.Tensor([65.481, 128.553, 24.966], dtype=mindspore.float32)
imagenet_mean = mindspore.Tensor([0.485, 0.456, 0.406], dtype=mindspore.float32).unsqueeze(1).unsqueeze(2)
imagenet_std = mindspore.Tensor([0.229, 0.224, 0.225], dtype=mindspore.float32).unsqueeze(1).unsqueeze(2)
imagenet_mean_cuda = mindspore.Tensor([0.485, 0.456, 0.406], dtype=mindspore.float32).unsqueeze(0).unsqueeze(2).unsqueeze(3)
imagenet_std_cuda = mindspore.Tensor([0.229, 0.224, 0.225], dtype=mindspore.float32).unsqueeze(0).unsqueeze(2).unsqueeze(3)
...
def create_data_lists(train_folders, test_folders, min_size, output_folder):
    ...
    for d in test_folders:
        ...
        with open(os.path.join(output_folder, test_name + '_test_images.json'), 'w', encoding='utf-8') as j:
...
def convert_image(img, source, target):
    ...
    assert target in {'pil', '[0, 255]', '[0, 1]', '[-1, 1]', 'imagenet-norm',
                      'y-channel'}, f"Cannot convert to target format {target}!"
    ...
    if target == 'pil':
        if source == 'pil':
            img = Image.fromarray(np.uint8(255 * VT.ToTensor()(img).transpose(1, 2, 0)))
        else:
            img = VT.ToPIL()(img)
        ...
        img = ops.matmul(255. * img.permute(0, 2, 3, 1)[:, 4:-4, 4:-4, :], rgb_weights) / 255. + 16.
