...
class SRDataset:
    """
    Dataset for MindSpore GeneratorDataset
    """
    ...
    def __init__(self, data_folder, split, crop_size, scaling_factor, lr_img_type, hr_img_type, test_data_name=None):
        ...
        :param scaling_factor: the input LR images will be downsampled from the target HR images by this factor;
        ...
        if self.split == 'train':
            with open(os.path.join(data_folder, 'train_images.json'), 'r', encoding='utf-8') as j:
                self.images = json.load(j)
        else:
            with open(os.path.join(data_folder, self.test_data_name + '_test_images.json'), 'r', encoding='utf-8') as j:
                self.images = json.load(j)
